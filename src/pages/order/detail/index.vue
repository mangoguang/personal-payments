...
import { fetchOrderDetail, fetchUpdateOrder } from '@/api/order'
import { dictType } from '@/utils/constants'
import { PublicPath } from '@/utils/axios'
...
export default {
  ...
  computed: {
    classifyValue: function (newVal, oldVal) {
  ...
  async onLoad (option) {
    ...
    await this.setSelectData()
    this.uploadImg = []
    this._getData(option.id)
  },
  methods: {
    async _getData (id) {
      let data = await fetchOrderDetail(id)
      // accountCode: "meal_card"
      // accountName: "饭卡"
      // createTime: "2021-05-06 16:14:36"
      // imgUrl: null
      // levelOneName: "食物酒水"
      // levelTwoName: "早午晚餐"
      // memberCode: ""
      ...
      // money: "12"
      // orderType: "0"
      // projectCode: "daliy_project"
      // projectName: "日常项目"
      // remark: ""
      this.money = data.money
      this.classifyType = [data.levelOneName, data.levelTwoName]
      this.accountType[1] = data.accountName
      this.changeTime = data.createTime
      this.memberType[1] = data.memberName || ''
      this.projectType[1] = data.projectName
      this.remark = data.remark
      data.imgUrl && this.uploadImg.push({ url: `${PublicPath}${data.imgUrl}`, name: 'picture' })
    },
    moneyChange (e) {
    ...
    async setSelectData () {
      ...
      console.log(1111, accountTypeData)
    ...
    update () {
      if (this.key) {
        ...
        let { money, classifyType, accountType, changeTime, memberType, projectType, remark } = this
        let params = { money, classifyType: classifyType[1], accountType: accountType[1], date: changeTime, memberType: memberType[1], projectType: projectType[1], remark }
        fetchUpdateOrder(this.orderId, params).then(() => {
          this.key = true
